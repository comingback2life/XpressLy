...
const server = app.listen(8080, () => {
   ...
   console.log("Server started http://%s:%s", host, port)
	...
	app.post('/users', userImageUpload.single('file'), (req, res) => {
		...
		if (user.username && user.password && user.fullName && user.email && user.image) {
	...
	app.post('/vendors', vendorImageUpload.single('file'), (req, res) => {
		...
		if (vendor.username && vendor.password && vendor.vendorName && vendor.email && vendor.image) {
			const sql = `insert into vendor (username, password, email, vendorName, availableBalance, holdBalance, image, status) values ('${vendor.username}', '${vendor.password}', '${vendor.email}', '${vendor.vendorName}', '0', '0', '${vendor.image}', 'ACTIVE')`;
			con.query(sql, (err, result) => {
				if (err) {
					console.log(err);
					res.status(400).json({
	...
	app.get('/organizations', jsonParser, (req, res) => {
		jwt.verify(req.headers['authorization'], req.headers['username'], (err, decoded) => {
			if (decoded) {
					...
					if (decoded) {
						...
						con.query(sql, (err, result) => {
							...
							con.query(query, (err, result) => {
								...
								con.query('select * from vendor',  (err, result) => {
									data.numberOfUsers = data.numberOfUsers + result.length;

									con.query('select * from organization', (err, query) => {
										data.numberOfUsers = data.numberOfUsers + result.length;
	...
	app.post('/organizations', orgImageUpload.single('file'), (req, res) => {
		...
		if (organization.name && organization.representativeFullName && organization.username && organization.password && organization.phoneNumber && organization.email && organization.image) { 
				const sql = `insert into organization (name, password, purposeStatement, representativeFullName, availableBalance, holdBalance, username, phoneNumber, email, image, status) values ('${organization.name}', '${organization.password}', '${organization.purposeStatement}', '${organization.representativeFullName}', '0', '0', '${organization.username}', '${organization.phoneNumber}', '${organization.email}', '${organization.image}', 'ACTIVE')`;
				con.query(sql, (err, result) => {
					if (err) {
						console.log(err)
						res.status(400).json({
			...
			if (err) {
				...
				con.query(`select * from transaction where id=${req.body.transactionId}`, (err, result) => {
					...
					con.query(`select * from ${type} where username='${username}'`, (err, result) => {
						...
						if (isWithdrawl) {
							holdBalance = Number(result[0].holdBalance) - Number(amount);
							amount = Number(result[0].availableBalance) - Number(amount);
							console.log(result[0].holdBalance)
							console.log(amount)
						} else {
							amount = Number(result[0].availableBalance) + Number(amount);
						}

						con.query(`update ${type} set availableBalance='${amount}',holdBalance=${holdBalance} where username='${username}'`, (err, result) => {
	...
	app.post('/activateUser', jsonParser, (req, res) => {
		...
		console.log(table);
		const sql = `update ${table} set status='ACTIVE' where username='${req.body.username}'`;
		con.query(sql, (err, result) => {
			if (err) {
				console.log(err)
				res.status(400).json({
	...
	app.post('/deactivateUser', jsonParser, (req, res) => {
		...
		console.log(table);
		const sql = `update ${table} set status='INACTIVE' where username='${req.body.username}'`;
		con.query(sql, (err, result) => {
			if (err) {
				console.log(err)
				res.status(400).json({
		...
		if (req.body.username && req.body.password) {
			...
			con.query(sql, (err, result) => {
				if (err || result.length === 0) {
					...
					if (result[0].status?.toLowerCase() === 'inactive') {
						res.status(400).send({'message': 'User is inactive'});
					} else {
						const token = jwt.sign({ username: req.body.username }, req.body.username, { expiresIn: "2h", });
						if (userType === 'vendor')
	...
	app.post('/users/send/org/', jsonParser, (req, res) => {
		...
		if (senderUsername && organizationId && amount) {
									...
									if (err) {
										...
										con.query(`update organization set availableBalance='${receiver.availableBalance}' where username='${organizationId}'`, (err, result) => {
											if (err) {
												...
												res.status(200).json({message: 'Money successfully sent', availableBalance: sender.availableBalance, holdBalance: sender.holdBalance});
												});
											}
										});
									}
	...
	app.post('/users/withdraw', jsonParser, (req, res) => {
		...
		if (username && vendorUsername && amount) {
	...
	app.post('/vendors/withdraw', jsonParser, (req, res) => {
		...
		if (vendorUsername && bankName && accountNumber && amount) {
			...
			con.query(`select * from vendor where username='${vendorUsername}'`, (err, result) => {
				if (err || result.length === 0) {
					...
					try {
						...
						if (Number(vendor.availableBalance) < amount) {
	...
	app.post('/organizations/withdraw', jsonParser, (req, res) => {
		...
		if (username && bankName && accountNumber && amount) {
			const vendorWithdrawUseCase = new VendorWithdrawUseCase();
			...
			con.query(`select * from organization where username='${username}'`, (err, result) => {
				if (err || result.length === 0) {
					...
					try {
						if (Number(organization.availableBalance) < amount) {
							console.log('here')
							throw new Error('Insufficient balance');
						}
						...
						con.query(`update organization set holdBalance=${organization.holdBalance} where username='${username}'`, (err, result) => {
							if (err) {
								console.log(err)
								throw new Error('Error updating hold balance');
							} else {
